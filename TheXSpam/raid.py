...
async def raid(xspam: Client, message: Message):  
      ...
      if len(alt) > 2:
            ...
            id = ok.id
            if id in THE_ALTS:
                  await message.reply_text("» ɴᴏ, 𝐓ʜɪꜱ ɢᴜʏ ɪꜱ ᴀʟᴛʀᴏɴ'ꜱ ᴏᴡɴᴇʀ")
            elif id == OWNER_ID:
                  await message.reply_text("» ɴᴏ, 𝐓ʜɪꜱ ɢᴜʏ ɪꜱ ᴏᴡɴᴇʀ ᴏꜰ ᴛʜᴇ ʙᴏᴛꜱ.")
            elif id in SUDO_USERS:
                  await message.reply_text("» ɴᴏ, 𝐓ʜɪꜱ ɢᴜʏ ɪꜱ ᴀ ꜱᴜᴅᴏ ᴜꜱᴇʀ")
            else:
                  counts = int(alt[1])
                  fname = ok.first_name
...
async def rraid(xspam: Client, message: Message):
      ...
      if len(alt) > 1:
          ...
          if id in THE_ALTS:
              ...
              rusers.append(id)
              await message.reply_text("» 𝐀ᴄᴛɪᴠᴀᴛᴇᴅ ʀᴇᴘʟʏʀᴀɪᴅ !! ✅")
...
async def draid(xspam: Client, message: Message):
      ...
      if len(alt) > 1:
          ...
          if id in rusers:
              rusers.remove(id)
              await message.reply_text("» 𝐑ᴇᴘʟʏ ʀᴀɪᴅ ᴅᴇ-ᴀᴄᴛɪᴠᴀᴛᴇᴅ !! ✅")
