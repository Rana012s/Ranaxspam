...
async def dmraid(xspam: Client, message: Message):
      ...
      if len(alt) == 3:
          ...
          if id in THE_ALTS:
                ...
                await message.reply_text(f"`𝐓ʜɪs ᴘᴇʀsᴏɴ ɪs ᴍʏ sᴜᴅᴏ ᴜsᴇʀ`")
          else:
              counts = int(alt[1])
              await message.reply_text("`𝐃ᴍ ʀᴀɪᴅ sᴛᴀʀᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ`")
          ...
          if id in THE_ALTS:
                await message.reply_text(f"`𝐕ᴇʀɪғɪᴇᴅ ʙʏ ᴀʟᴛʀᴏɴ x`")
          elif id in SUDO_USERS:
                await message.reply_text(f"`𝐓ʜɪs ᴘᴇʀsᴏɴ ɪs ᴍʏ sᴜᴅᴏ ᴜsᴇʀ`")
          else:
              counts = int(alt[1])
...
async def dmspam(client: Client, message: Message):
    ...
    if  len(alt) == 4:
        ...
        if uid in THE_ALTS:
            ...
            await message.reply_text("`𝐃ᴍ ꜱᴘᴀᴍ sᴛᴀʀᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ`")
        ...
        await message.reply_text("😈 ᴜsᴀɢᴇ:\n .dmspam 13 <ᴜꜱᴇʀ ɪᴅ> Rana\n .dmspam 13 Rana <ʀᴇᴘʟʏ ᴛᴏ ᴜsᴇʀ>")
